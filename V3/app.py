...
import numpy as np
import os
import ctypes  # For DPI scaling on Windows

# Enable DPI scaling (improves UI sharpness)
...
def convert_audio():
    ...
    elif pattern == "Circular Rotation":
        angle = 2.0 * np.pi * t * speed
        left_channel = samples[:, 0] * (0.5 * (1 + np.cos(angle)))
        right_channel = samples[:, 1] * (0.5 * (1 - np.cos(angle)))

    elif pattern == "Wave":
        wave_speed = 2 * np.pi * speed
        left_channel = samples[:, 0] * (0.5 * (1 + np.sin(wave_speed * t)))
        right_channel = samples[:, 1] * (0.5 * (1 - np.sin(wave_speed * t)))

    elif pattern == "Heartbeat":
        heartbeat_speed = speed * 2
        pulse = np.sin(2 * np.pi * heartbeat_speed * t) * 0.5 + 0.5
        left_channel = samples[:, 0] * pulse
        right_channel = samples[:, 1] * pulse

    elif pattern == "Zig-Zag":
        zigzag_speed = speed * 10
        panning = np.sin(zigzag_speed * t)
        left_channel = samples[:, 0] * (0.5 * (1 + panning))
        right_channel = samples[:, 1] * (0.5 * (1 - panning))

    elif pattern == "LFO":
        lfo_speed = 0.5  # Low-frequency oscillation speed
        lfo = np.sin(2 * np.pi * lfo_speed * t) * 0.5 + 0.5
        left_channel = samples[:, 0] * lfo
        right_channel = samples[:, 1] * lfo
...
# Apply the theme
style = Style(theme='simplex')  # Try 'superhero', 'journal', 'cosmo', 'flatly', 'litera', 'minty',
                                # 'lumen', 'sandstone', 'yeti', 'pulse', 'united', 'morph', 'journal',
...
# File selection
label = ttk.Label(frame, text="Upload Audio File:", font=("Arial", 11, "bold"))
label.grid(row=0, column=0, padx=5, pady=5, sticky=tk.W)

entry_file_path = ttk.Entry(frame, width=55)
...
# Convert Button
btn_convert = ttk.Button(frame, text="Convert to 3D/8D", command=convert_audio, style="primary.TButton")
btn_convert.grid(row=1, column=0, columnspan=3, pady=20, sticky=tk.W)

# Speed control
speed_label_var = tk.StringVar(value="Speed: 1.00")

label_speed = ttk.Label(frame, text="Adjust Speed:", font=("Arial", 11, "bold"))
label_speed.grid(row=2, column=0, padx=5, pady=5, sticky=tk.W)

speed_scale = ttk.Scale(frame, from_=0.25, to=2, orient='horizontal', length=300, command=update_speed_label)
speed_scale.set(1)
speed_scale.grid(row=2, column=1, columnspan=2, padx=5, pady=5)

speed_label = ttk.Label(frame, textvariable=speed_label_var, font=("Arial", 10))
speed_label.grid(row=3, column=0, columnspan=3, pady=5, sticky=tk.W)

# Pattern selection
label_pattern = ttk.Label(frame, text="Select Pattern:", font=("Arial", 11, "bold"))
label_pattern.grid(row=4, column=0, padx=5, pady=5, sticky=tk.W)

patterns = ["Circular", "Sine", "Random Pan", "Echo Effect", "Circular Rotation", "Wave", "Heartbeat", "Zig-Zag", "LFO"]
pattern_var = ttk.Combobox(frame, values=patterns, state="readonly", width=30)
pattern_var.set(patterns[0])  # Default to "Circular"
pattern_var.grid(row=4, column=1, padx=5, pady=5, sticky=tk.W)
